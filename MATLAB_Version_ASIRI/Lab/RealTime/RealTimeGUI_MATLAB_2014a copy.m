...
FCTD_SerialStream.OutputBufferSize = 1024*4;
...
DATA = fftshift(fft(detrend(data)))/N_pts*2;
...
% get(handles.uCOND);
% set(handles.uCond,'xaxislocation','top');
% 
% set(handles.uCOND,'interpreter','latex');
% 
% set(handles.cond,'interpreter','latex');
% 
% set(handles.temp,'interpreter','latex');
...
%     s = fread(FCTD_SerialStream,FCTD_SerialStream.BytesAvailable,'char*');
% end

FID=fopen('~/tmp/FCTD/FCTD_Data_Test_uCond_w_alt_20150115.txt','r');
s = fread(FID,'*char');
fclose(FID);
...
altTime = FastCTD_ASCII_getAltimeter(s(:,ctdpos+ctdlen+mcrlen+acclen+(1:altlen)-1));
